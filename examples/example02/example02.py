...
def plot_surface():
    ...
    surface_data = pd.read_csv('./output/shaft.csv', on_bad_lines='skip', low_memory=False)

    labels = ['drill', 'lathe', 'polisher']

    for index, label in enumerate(labels):
        plt.plot(surface_data[surface_data['station_id'] == str(index)]['time'],
                 surface_data[surface_data['station_id'] == str(index)]['surface'], label=label)
